...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected String toChatUsername, remarkname;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        remarkname = fragmentArgs.getString(EaseConstant.EXTRA_REMARKNAME);
    ...
    protected void initView() {
        ...
        inputMenu.init(null);
        if (isShare) {
            sendTextMessage(getString(R.string.fenxiangyishengmingpian));
        }
    ...
    public void onMessageReceived(List<EMMessage> messages) {
        ...
        for (EMMessage message : messages) {
            ...
            if (username.equals(toChatUsername)) {
                ...
                Log.e("EaseChatFragment 收到消息", message.getUserName() + ":" + message.getBody());
    ...
    protected void sendTextMessage(String content) {
        if (EaseAtMessageHelper.get().containsAtUsername(content)) {
            ...
            // 增加自己特定的属性
            addSelfAttr(message);
//            Log.e("jujing 713", BaseApplication.username + "###" + BaseApplication.headurl);
            if (sendMessageLinstener != null) {
                sendMessageLinstener.onSendMessage(message);
            }
        }
    }
     ...
     * 增加自己特定的属性
     */
    public void addSelfAttr(EMMessage message) {
        if (isShare) {
            //增加自己的属性，用于自定义的消息类型 分享时用到
            message.setAttribute(EaseConstant.MESSAGE_ATTR_IS_SHARE_CALL, true);
            if (docotrInfo != null) {
                message.setAttribute(EaseConstant.MESSAGE_SHARE_HEADER_URL, docotrInfo.getHeadUrl());
                message.setAttribute(EaseConstant.MESSAGE_SECTION, docotrInfo.getSection());
                message.setAttribute(EaseConstant.MESSAGE_SHARE_ID, docotrInfo.getUserId());
                message.setAttribute(EaseConstant.MESSAGE_SHARE_NAME, docotrInfo.getName());
                message.setAttribute(EaseConstant.MESSAGE_TITLE, docotrInfo.getJob());
                message.setAttribute(EaseConstant.MESSAGE_GOODAT, docotrInfo.getDisease());
            }
        } else {
            message.setAttribute(EaseConstant.MESSAGE_ATTR_IS_SHARE_CALL, false);
        }
        message.setAttribute("name", BaseApplication.username);
        message.setAttribute("userID", BaseApplication.userId);//对方的userId
        message.setAttribute("logo", BaseApplication.headurl);
    }
    ...
    private void sendAtMessage(String content) {
        ...
        // 增加自己特定的属性
        message.setAttribute("name", BaseApplication.username);
        message.setAttribute("userID", BaseApplication.userId);
        message.setAttribute("logo", BaseApplication.headurl);
...
//        sendMessage(message);
    ...
    protected void selectPicFromCamera() {
        ...
        boolean flag = PermissionChecker.checkSelfPermission(getActivity(), Manifest.permission.CAMERA) == PermissionChecker.PERMISSION_GRANTED;
        if (!flag) {
            new MaterialDialog(getActivity())
                    .setMDTitle(getString(R.string.zhongyaotishi))
                    .setMDMessage(getString(R.string.xuyaokaiqishexiangtouquanxian))
    ...
    public interface SendMessageLinstener {
        void onSendMessage(EMMessage message);
    }

    private SendMessageLinstener sendMessageLinstener;

    public void setSendMessageLinstener(SendMessageLinstener sendMessageLinstener) {
        this.sendMessageLinstener = sendMessageLinstener;
    }
