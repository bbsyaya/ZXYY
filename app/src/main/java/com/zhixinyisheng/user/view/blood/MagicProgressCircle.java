...
public class MagicProgressCircle extends View implements ISmoothTarget {
	...
	protected void onDraw(Canvas canvas) {
		...
		// 背景圆
		canvas.save();
		canvas.drawArc(rectF1, -235, 290, true, paint1);
		canvas.restore();
		// 背景白园
		canvas.save();
		canvas.drawArc(rectF2, -235, 360, true, paint2);
		canvas.restore();
		// 绘制开始的半圆
		canvas.save();
		canvas.rotate(-145, cx, cy);
		canvas.drawArc(rectF, 90f, 180f, true, paint1);
		canvas.restore();
		// 绘制结束的半圆
		canvas.save();
		endPaint.setColor(percentEndColor);
		canvas.rotate(-216, cx, cy);
		canvas.drawArc(rectF, -90f, 180f, true, paint1);
		canvas.restore();

		// 画渐变圆
		canvas.save();
		canvas.rotate(-235, cx, cy);

		if (drawPercent > 290 / 360f) {
			drawPercent = 290 / 360f;
		}
		if (drawPercent <= 290 / 360f && drawPercent >= 0) {
		...
		if (drawPercent > 0) {
			...
			if (drawPercent <= 290 / 360f) {
				...
				endPaint.setColor(percentEndColor);
				canvas.rotate(
						((int) Math.floor(360.0f * drawPercent) - 1) - 145, cx,
						cy);
