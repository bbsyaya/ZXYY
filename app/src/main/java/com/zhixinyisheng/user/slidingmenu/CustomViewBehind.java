package com.zhixinyisheng.user.slidingmenu;
...
import com.zhixinyisheng.user.slidingmenu.SlidingMenu.CanvasTransformer;
...
public class CustomViewBehind extends ViewGroup {
	...
	public void drawOffsetFade(View content, Canvas canvas, float openPercent) {
		if (mWidthOffset <= 0 || mFadeDegree <= 0) return;   //不开启就返回，不设置 淡入淡出效果

		final int alpha = (int) (mFadeDegree * 200 * Math.abs(openPercent));
		mFadePaint.setColor(Color.argb(alpha, 0, 0, 0));
