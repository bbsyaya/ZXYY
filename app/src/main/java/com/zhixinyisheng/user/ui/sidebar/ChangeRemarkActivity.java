...
public class ChangeRemarkActivity extends BaseAty {
    @Bind(R.id.title)
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        cjsTvt.setText(R.string.genggaigexingqianming);
        titleBtn.setText(R.string.finish);
        String content = UserManager.getUserInfo().getRemark();
        etChangeRemark.setText(content);
        etChangeRemark.requestFocus();
        try {
            etChangeRemark.setSelection(content.length());
        } catch (Exception e) {
            e.printStackTrace();//新版可以删掉这个 try - catch
        }
    }
    ...
    public void onSuccess(String result, Call<ResponseBody> call, Response<ResponseBody> response, int what) {
        ...
        userInfo.setRemark(etChangeRemark.getText().toString().trim());
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.cjs_rlb:
                ...
                break;
            case R.id.title_btn:
                if (etChangeRemark.getText().toString().trim().equals("")) {
                    showToast(getString(R.string.qingshurugexingqianming));
                    return;
                }
                showLoadingDialog(null);
                doHttp(RetrofitUtils.createApi(LeftUrl.class).personinformation(phone, secret, phone, null,
                        null, null, null,
                        null, etChangeRemark.getText().toString().trim(),
